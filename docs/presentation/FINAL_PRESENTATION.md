...
### 메인 모델: Ridge Regression
```
모델 타입:     Ridge (alpha=1.0)
타겟 변수:     target_vol_5d (5일 후 변동성)
검증 방법:     Purged K-Fold CV (5-fold)
샘플 수:       2,460개 (2015-2024 SPY 실제 데이터)
특성 수:       31개 (변동성 + 래그 변수)
```
...
| 지표 | 값 | 설명 |
|------|-----|------|
| **CV R²** | **0.303** | Cross-validation 평균 (30.3%) |
| **CV Std** | 0.198 | 폴드 간 표준편차 |
| **Test MAE** | 0.00332 | 평균 절대 오차 |
| **Test RMSE** | 0.00530 | 평균 제곱근 오차 |

**해석**: 변동성의 30.3%를 설명 가능 → 리스크 관리에 유의미한 수준
...
| 모델 | R² | RMSE | MAE | 특성 수 | 비고 |
|------|-------|---------|--------|---------|------|
| **HAR** | 0.215 | 0.00318 | 0.00248 | 3 | 학술 표준 벤치마크 |
| **Ridge** | **0.303** | 0.00530 | 0.00332 | 31 | **1.41배 개선** |

### 성능 개선도
- **R² 향상**: 0.088 포인트 (8.8%p)
- **설명력 비율**: 1.41배 (41% 더 높음)
- **결론**: Ridge 모델이 HAR 대비 통계적으로 유의미하게 우수
...
### 주요 발견사항
1. **수익률**: RV 전략이 Buy & Hold 대비 0.97%p 높음
2. **변동성**: 다소 증가 (2.55%p) - 공격적 포지션 조정 영향
3. **리스크 조정 수익**: 샤프 비율 유사 (0.520 vs 0.541)
4. **낙폭 관리**: V0 대비 개선 (-5.27%p)

**결론**: 변동성 예측이 리스크 관리에 실질적 기여 가능
