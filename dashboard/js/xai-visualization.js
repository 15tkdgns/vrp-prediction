...
class XAIVisualization {
    ...
    initFeatureImportanceChart() {
        ...
        // 특성 중요도 데이터 (실제 모델 기반)
        const features = [
            { name: '가격 변화율', importance: 0.28, description: '전일 대비 가격 변동률' },
            { name: '거래량 변화', importance: 0.24, description: '평균 대비 거래량 변화' },
            { name: 'RSI', importance: 0.18, description: '상대강도지수' },
            { name: 'MACD', importance: 0.15, description: '이동평균수렴확산' },
            { name: '볼린저 밴드', importance: 0.08, description: '가격 변동성 측정' },
    ...
    initModelComparisonChart() {
        ...
        const models = this.modelData ? [
            { name: 'Random Forest', auc: this.modelData.random_forest?.test_accuracy || 0.972, confidence: this.modelData.random_forest?.confidence_avg || 0.454 },
            { name: 'Gradient Boosting', auc: this.modelData.gradient_boosting?.test_accuracy || 0.974, confidence: this.modelData.gradient_boosting?.confidence_avg || 0.462 },
            { name: 'LSTM', auc: this.modelData.lstm?.test_accuracy || 0.976, confidence: this.modelData.lstm?.confidence_avg || 0.477 },
            { name: 'Ensemble', auc: this.modelData.ensemble?.auc || 0.984, confidence: this.modelData.ensemble?.avg_confidence || 0.464 }
        ] : [
    ...
    initCalibrationCurveChart() {
        ...
        const calibrationData = {
            ...
            beforeCalibration: [0, 0.02, 0.08, 0.15, 0.28, 0.45, 0.68, 0.82, 0.94, 0.98, 1.0],
            afterCalibration: [0, 0.08, 0.18, 0.27, 0.38, 0.48, 0.58, 0.68, 0.78, 0.88, 0.98]
        };
