...
def create_tft_datasets(df, max_encoder_length=30, max_prediction_length=5):
    ...
    # 검증 데이터셋 (predict=False로 변경하여 더 많은 샘플 확보)
    validation = TimeSeriesDataSet.from_dataset(
        training, 
        df.iloc[split_idx:], 
        predict=False,  # 더 많은 샘플 생성
...
def build_tft_model(training):
    ...
    tft = TemporalFusionTransformer.from_dataset(
        ...
        log_interval=-1,  # 로깅 비활성화
        log_val_interval=-1,  # 검증 로깅 비활성화
    )
    ...
    try:
        ...
        if hasattr(pred_values, 'shape'):
            ...
            if len(pred_values.shape) == 3:
                pred_median = pred_values[:, 0, pred_values.shape[2] // 2]  # 첫 스텝, 중간 quantile
            elif len(pred_values.shape) == 2:
                pred_median = pred_values[:, 0]
            else:
                pred_median = pred_values
        else:
            pred_median = torch.tensor([p.mean().item() for p in pred_values])
        
        # 실제 값 (첫 스텝)
        y_true = actuals[:, 0].cpu().numpy()
        y_pred = pred_median.cpu().numpy().flatten()
        ...
        import traceback
        traceback.print_exc()
        return {'test_r2': 0.0, 'error': str(e)}
